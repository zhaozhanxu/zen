...
def print_error(message):
    global __errors
    print("\033[91mERROR: %s\033[00m" % message)
...
def print_warning(message):
    global __warnings
    print("\033[93mWARNING: %s\033[00m" % message)
...
__parenthesized_constructs = 'if|for|while|switch|[_A-Z]+FOR_EACH[_A-Z]*|[_a-z]+for_each[_a-z]*'
...
std_functions = [
        ('rte_panic', 'Use zen_panic() in place of rte_panic()'),
        ('rte_log', 'Use zen_log() in place of rte_log()'),
]
checks += [
    {'regex': '(.c|.h)(.in)?$',
     'match_name': None,
...
def check_patch_parse(text, filename):
...
def check_patch_print_result(result):
...
def check_patch_file(filename):
    ...
    for part in mail.walk():
        if part.get_content_maintype() == 'multipart':
            continue
    result = check_patch_parse(part.get_payload(decode=False), filename)
    check_patch_print_result(result)
    return result
...
if __name__ == '__main__':
    try:
        ...
        optlist, args = getopt.getopt(args, 'hf',
                                      ["check-file",
                                       "help"])
    except:
    ...
    if n_patches:
        ...
        for i in reversed(range(0, n_patches)):
            ...
            result = check_patch_parse(patch, revision)
            check_patch_print_result(result)
            if result:
                status = -1
        sys.exit(status)
    ...
    if not args:
        if sys.stdin.isatty():
            usage()
            sys.exit(-1)
        result = check_patch_parse(sys.stdin.read(), '-')
        check_patch_print_result(result)
        sys.exit(result)
    ...
    for filename in args:
        ...
        result = check_patch_file(filename)
        if result:
            status = -1
    sys.exit(status)
